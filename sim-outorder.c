...
#include "machine.def"
  ...
  for (int huron = 0;;huron++)
	...
	ruu_fetch_issue_delay--;
      ...
      /* go to next cycle */
      sim_cycle++;
      if(sim_cycle > 0 && sim_num_insn > 0 && sim_num_insn % 10000000 == 0){
        FILE *fp = NULL;
        float il1_miss_rate = (float)cache_il1->misses / ((float)cache_il1->misses + (float)cache_il1->hits);
        char runtime_il1_miss_rate[255];
        sprintf(runtime_il1_miss_rate,"%f",il1_miss_rate);
        fp =fopen("runtime/il1.txt","a+");
        fputs(runtime_il1_miss_rate,fp);
        fclose(fp);

        float il2_miss_rate = (float)cache_il2->misses / ((float)cache_il2->misses + (float)cache_il2->hits);
        char runtime_il2_miss_rate[255];
        sprintf(runtime_il2_miss_rate,"%f",il2_miss_rate);
        fp =fopen("runtime/il2.txt","a+");
        fputs(runtime_il2_miss_rate,fp);
        fclose(fp);

        float dl1_miss_rate = (float)cache_dl1->misses / ((float)cache_dl1->misses + (float)cache_dl1->hits);
        char runtime_dl1_miss_rate[255];
        sprintf(runtime_dl1_miss_rate,"%f",dl1_miss_rate);
        fp =fopen("runtime/dl1.txt","a+");
        fputs(runtime_dl1_miss_rate,fp);
        fclose(fp);

        float dl2_miss_rate = (float)cache_dl2->misses / ((float)cache_dl2->misses + (float)cache_dl2->hits);
        char runtime_dl2_miss_rate[255];
        sprintf(runtime_dl2_miss_rate,"%f",dl2_miss_rate);
        fp =fopen("runtime/dl2.txt","a+");
        fputs(runtime_dl2_miss_rate,fp);
        fclose(fp);

        float mem_ptab_miss_rate = (float)mem->ptab_misses / (float)mem->ptab_accesses;
        char run_time_mem_ptab_miss_rate[255];
        sprintf(run_time_mem_ptab_miss_rate,"%f",mem_ptab_miss_rate);
        fp =fopen("runtime/mory.txt","a+");
        fputs(run_time_mem_ptab_miss_rate,fp);
        fclose(fp);

        float run_time_IPC = (float)sim_num_insn/(float)sim_cycle;
        char IPC_huron[255];
        sprintf(IPC_huron,"%f",run_time_IPC);
        fp = fopen("runtime/IPC.txt","a+");
        //fputs("\nThe current IPC is \n",fp);
        fputs(IPC_huron,fp);
        //fputs("\n",fp);
        fclose(fp);

        float bpred_bimod_misses = (float)pred->misses/(float)(pred->addr_hits+pred->misses);
        char runtime_bpred_bimod_misses[255];
        sprintf(runtime_bpred_bimod_misses,"%f",bpred_bimod_misses);
        fp =fopen("runtime/bpred.txt","a+");
        fputs(runtime_bpred_bimod_misses,fp);
        fclose(fp);

        /*FILE *fp = NULL;
        fp = fopen("runtime_performance.txt","a+");
        //fputs("\nThe current IPC is \n",fp);
        fputs(IPC_huron,fp);
        fputs("\n",fp);

        //fputs("the current memory page table miss rate is \n",fp);
        fputs(run_time_mem_ptab_miss_rate,fp);
        fputs("\n",fp);

        //fputs("the current il1 miss rate is \n",fp);
        fputs(runtime_il1_miss_rate,fp);
        fputs("\n",fp);

        //fputs("the current il2 miss rate is \n",fp);
        fputs(runtime_il2_miss_rate,fp);
        fputs("\n",fp);

        //fputs("the current dl1 miss rate is \n",fp);
        fputs(runtime_dl1_miss_rate,fp);
        fputs("\n",fp);

        //fputs("the current dl2 miss rate is \n",fp);
        fputs(runtime_dl2_miss_rate,fp);
        fputs("\n",fp);

        //fputs("the current branch miss prediction rate is \n",fp);
        fputs(runtime_bpred_bimod_misses,fp);
        //fputs("\n",fp);
        fputs("\n",fp);
